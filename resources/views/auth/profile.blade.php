<x-layouts.app>
    <div class="flex items-center justify-center min-h-screen bg-cover bg-center bg-no-repeat bg-[url('/public/img/homeImage.jpeg')] bg-gray-400 bg-blend-multiply" >
        <div class="w-full max-w-sm p-8 bg-white rounded-lg shadow-lg bg-opacity-60 backdrop-blur-lg">
            <div class="flex flex-col items-center justify-center mb-5">
                <img class="rounded-full w-30" src="{{ asset( 'storage/'.Auth::user()->image) }}" alt="image description">
            </div>
    ...
    <section class="p-3 my-20 bg-gray-50 dark:bg-gray-900 sm:p-5">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                <th scope="col" class="px-4 py-3">Property</th>
                            ...
                            @foreach ($reservations as $reservation)
                                ...
                                <tr class="border-b dark:border-gray-700">
                                    <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        <h5>{{$reservation->room->property->name}}</h5>
                                        <p>{{$reservation->room->name}}</p>
                                    </th>
                                    <td class="px-4 py-3">
                                        <p>{{$reservation->check_in}} - {{$reservation->check_out}}</p>
                                        <p>{{(int)$reservation->hours}} Hour</p>
                                    </td>
                                    <td class="px-4 py-3">Rp.300.000,00</td>
                                    <td class="px-4 py-3">Rp.600.000,00</td>
                                    <td class="px-4 py-3">
                                        ...
                                        @elseif ($reservation->transaction->status == 'success')
                                            <span class="bg-green-100 text-green-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded-sm dark:bg-green-900 dark:text-green-300">Success</span>
                                        @elseif ($reservation->transaction->status == 'failed' || $reservation->transaction->status == 'canceled')
                                            <span class="bg-red-100 text-red-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded-sm dark:bg-red-900 dark:text-red-300">Failed</span>
                                        @elseif ($reservation->transaction->status == 'expired')
                                            <span class="bg-red-100 text-red-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded-sm dark:bg-red-900 dark:text-red-300">Expired</span>
                                        @endif
                                    </td>
