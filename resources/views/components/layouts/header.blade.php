    ...
    <nav class="border-gray-200 px-4 lg:px-6 py-2.5 ">
        <div class="flex flex-wrap items-center justify-between max-w-screen-xl mx-auto">
            ...
            <div class="flex items-center lg:order-2">
                ...
                @if (Auth::check())
                    <a  href="/profile" class="flex flex-row justify-items-center  items-center gap-2  dark:text-white  hover:text-cyan-500 focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-sm px-4 lg:px-5 py-2 lg:py-2.5 mr-2 dark:hover:bg-gray-700 focus:outline-none dark:focus:ring-gray-800">
                        <img class="w-8 rounded-full" src="{{ asset(Auth::user()->image) }}" alt="i{{ Auth::user()->name }}" />
                        <p id="user-info" class="text-white">
                            {{ Auth::user()->name }}
                        </p>
                    </a>
                @else
                    <a id="login-button" href="/auth/login" class="text-white dark:text-white  hover:text-cyan-500 focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-sm px-4 lg:px-5 py-2 lg:py-2.5 mr-2 dark:hover:bg-gray-700 focus:outline-none dark:focus:ring-gray-800">Log in</a>
                @endif
